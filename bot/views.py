...
try:
    users=Telegram_users.objects.filter(block=False)
    startbutton = [
        [KeyboardButton("Старт")],
    ]
    startbutton_reply = ReplyKeyboardMarkup(startbutton, resize_keyboard=True)
    for item in users:
        try:
            bot.send_message(item.user_id, text='Бот перезапущен', reply_markup=startbutton_reply)
        except:
            pass
...
def process_message(json_data):
    ...
    if message_text == '/start' or message_text == "Старт":
